function analysis_f0
    ...
    for i=1:2
        ...
        %% Get f0 data
        dataname = datainfo_i.dataname;
        
        %% Density estimation
        %D = helper.h_subsampling(helper.h_ETL_f0(dataname, f0dir), 4096);
    ...
    outputfileid = '';
    discreteness = pitchdiscreteness(f0dir, onsetdir, dataname, datainfo.type, typelist, datainfo.language, outputfileid, outputdir);

    %% Delta F0
    dataname = datainfo.dataname;
    outputfileid = '';
    deltaF0(f0dir, discreteness, dataname, datainfo.type, typelist, outputfileid, outputdir);
end

function deltaF0(f0dir, discreteness, dataname, datatype, typelist, outputfileid, outputdir)
    %% Delta F0 - setup
    pprm = plotprm();
    reffreq = 440;
    ...
    for i=1:numel(typelist)
        ...
        for j=1:numel(idx)
            ...
            %%
            f0info = readtable(strcat(f0dir, dataname{idx(j)}, '_f0.csv'));
    ...
    L = 64;
    h_L = zeros(L, 1);
    x = (dlt*round(min(cellfun(@(X) min(cat(1, X{:})), D))/dlt) - 100 - dlt/2):dlt:(dlt*round(max(cellfun(@(X) max(cat(1, X{:})), D))/dlt) + 100 + dlt/2);
    f_D = cell(numel(typelist), 1);
    C = cell(numel(typelist), 1);
    
    %% KDE - computation
    for i=1:numel(typelist)
        f_D{i} = zeros(numel(x), numel(D{i}));
        C{i} = zeros(numel(D{i}), 1);
        ...
        for j=1:numel(D{i})
            ...
            f_D{i}(:, j) = density;
            C{i}(j) = trapz(x, f_D{i}(:, j));

            fprintf('%s (%d): h = %3.3f\n', typelist{i},j,  h);
        end

        f_D{i} = bsxfun(@rdivide, f_D{i}, C{i}');
    end
    %}
    ...
    for i=1:numel(typelist)
        deltavar{i} = cellfun(@(X) mad(X, 1), D{i});
    ...
    ylabel('Pitch discreteness x Var[\DeltaF0] x median(F0)^{-1}', 'FontSize', pprm.labelfontsize);
    ax = gca(figobj);
...
function discreteness = pitchdiscreteness(f0dir, onsetdir, dataname, datatype, typelist, datalang, outputfileid, outputdir)
    %% Entropy - setup
    pprm = plotprm();
    reffreq = 440;
    ...
    for i=1:numel(typelist)
        ...
        for j=1:numel(idx)
            ...
            %%
            onsetinfo = readtable(strcat(onsetdir, 'onset_', dataname{i}, '.csv'));
            breakinfo = readtable(strcat(onsetdir, 'break_', dataname{i}, '.csv'));
